...
def token_to_id(tokenizer, max_seq_len):
    ...
    def get_id(samples: dict) -> dict:
        ...
        outputs = tokenizer(batch_txt, padding="max_length", return_attention_mask=False,
...
def get_maped_dataset(files, tokenizer, max_seq_len) -> Dataset:
    dataset = load_dataset(path="parquet", data_files=files, split="train",
                           cache_dir=TEMP_ROOT + "data/.cache", keep_in_memory=False)
    maped_dataset = dataset.map(token_to_id(tokenizer, max_seq_len), batched=True, batch_size=128,
...
if __name__ == "__main__":
    ...
    config = configuation_miniPhi3.MiniPhiConfig(attn_implementation=pretrain_config.attn_implementation, use_cope=False)
    ...
    train_dataset = get_maped_dataset(pretrain_config.train_files, tokenizer, pretrain_config.max_seq_len)
    eval_dataset = get_maped_dataset(pretrain_config.eval_files, tokenizer, pretrain_config.max_seq_len)
    ...
    train_args = TrainingArguments(
        ...
        optim=pretrain_config.optim,          # 使用配置中的优化器
